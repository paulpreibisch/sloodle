...
* <strong>NOTE:</strong> in Sloodle debug mode, this function will terminate the script with an error if the name is not prefixed with "sloodle_".
...
* <strong>NOTE:</strong> does not check for the presence of the Sloodle files.
...
* @todo <strong>May require update?</strong>
*/
/*
...
// Fumi.Hax
function sloodle_get_event($courseid, $action, $pgname='', $params=array(), $info='')
{
    global $CFG, $PAGE;
    ...
    $params = array(
        ...
        'other' => array('pgname'=>$pgname, 'params' => $params, 'info'=> $info),
    );

    //
    if ($action=='module_viewed') {
        $event = \mod_sloodle\event\module_viewed::create($params);
    }
    else if ($action=='blog_posted') {
        $event = \mod_sloodle\event\blog_posted::create($params);
    }
    else if ($action=='media_login') {
        $event = \mod_sloodle\event\media_login::create($params);
    }
    else if ($action=='mod_log') {
        $event = \mod_sloodle\event\mod_log::create($params);
    }
    else if ($action=='mail_log') {
        $event = \mod_sloodle\event\mail_log::create($params);
    }
    else if ($action=='user_log') {
        $event = \mod_sloodle\event\user_log::create($params);
    }
    else if ($action=='classroom_log') {
        $event = \mod_sloodle\event\classroom_log::create($params);
    }
...
// Fumi.Hax
function sloodle_add_to_log($courseid, $action, $pgname, $params=array(), $info='')
{
    global $CFG;

    $ret = false;

    if ($CFG->version>=2014051200) {    // v2.7
        $event = sloodle_get_event($courseid, $action, $pgname, $params, $info);
        if ($event!=null) {
            $event->trigger();
            $ret = true;
        }
        ...
        $url = new moodle_url('/mod/sloodle/'.$pgname, $params);
        add_to_log($courseid, 'sloodle', $action, $url->out(), $info);
        $ret = true;
    }
...
function sloodle_validate_prim_password_verbose($password, &$errors)
{
    ...
    // Check that it's a string
    if (!is_string($password) and !is_numeric($password)) {
        $errors[] = 'invalidtype';
        $result = false;
    }
    // Check the length
...
function sloodle_process_login_notifications($limit = 25)
{
    ...
    for ($i = 0; $i < $limit; $i++) {
        ...
        if (sloodle_send_login_notification($rec->destination, $rec->avatar, $rec->username, $rec->password)) {
            ...
            sloodle_add_to_log(SITEID, 'module_viewed', '', array(), 'Sent login details by email to avatar in-world');
        }
        else {
            ...
            sloodle_add_to_log(SITEID, 'module_viewed', '', array(), 'Failed to send login details by email to avatar in-world');
        }
...
function sloodle_print_access_level_options($current_config, $show_use_object = true, $show_control_object = true, $show_server = true)
{
    ...
    if ($show_use_object || $show_control_object) {
        ...
        // Object access
        echo '<strong>'.get_string('accesslevelobject','sloodle').'</strong><br /><i>'.get_string('accesslevelobject:desc','sloodle').'</i><br /><br />';
        // Use object
        if ($show_use_object) {
            ...
            echo html_writer::select($object_access_levels, 'sloodleobjectaccessleveluse', $sloodleobjectaccessleveluse, array());
            echo '<br /><br />';
        }
        // Control object
        if ($show_control_object) {
            ...
            echo html_writer::select($object_access_levels, 'sloodleobjectaccesslevelctrl', $sloodleobjectaccesslevelctrl, array());
            echo '<br /><br />';
        }
    }
    ...
    if ($show_server) {
        // Server access
        echo '<strong>'.get_string('accesslevelserver','sloodle').'</strong><br /><i>'.get_string('accesslevelserver:desc','sloodle').'</i><br /><br />';
        ...
        echo html_writer::select($server_access_levels, 'sloodleserveraccesslevel', $sloodleserveraccesslevel, array());
        echo '<br />';
    }        
...
function sloodle_access_level_option_choice($option, $current_config, $show, $prefix = '', $suffix = '')
{
    ...
    if ($show) {
        ...
        return html_writer::select($access_levels, $prefix.$option.$suffix, $selected_value, array());
    }
    else {
        return '&nbsp;';
    } 
...
function sloodle_view($feature)
{
    ...
    $any_id = optional_param('id', '-1', PARAM_INT);
    //
    $urlparams = array();
    $urlparams['_type']  = $feature;
    $urlparams['course'] = $course_id;
    if ($any_id!=-1) $urlparams['id'] = $any_id;
    $PAGE->set_url('/mod/sloodle/view.php', $urlparams);
